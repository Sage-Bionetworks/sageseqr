...
test_that("errors if duplicate sampleids in metadata", {
  expect_error(check_mismatch(metadata, counts))
})

### Duplicates in counts ###
metadata <- data.frame(
  batch = c("1", "2", "1", "2"),
  diagnosis = c("dx", "dx", "ct", "ct"),
  row.names = c("S1", "S2", "S3", "S4"),
...
test_that("errors if duplicate sampleids in counts matrix", {
  expect_error(check_mismatch(metadata, counts))
})