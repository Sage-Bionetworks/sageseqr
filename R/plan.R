...
rnaseq_plan <- function(metadata_id, metadata_version, counts_id,
                        ...
                        organism){
  drake::drake_plan(
    import_metadata = sageseqr::get_data(!!metadata_id,
                               !!metadata_version),
    import_counts = sageseqr::get_data(!!counts_id,
                             !!counts_version),
    counts = tibble::column_to_rownames(import_counts,
                                        var = !!gene_id_input),
    clean_md = sageseqr::clean_covariates(md = import_metadata,
                                factors = !!factor_input,
                                continuous = !!continuous_input),
    biomart_results = sageseqr::get_biomart(count_df = counts,
                                gene_id = !!gene_id,
                                synid = !!biomart_id,
                                ...
                                host = !!host,
                                organism = !!organism),
    filtered_counts = sageseqr::filter_genes(md = clean_md,
                                   count_df = counts),
    cqn_counts = sageseqr::cqn(filtered_counts,
                     biomart_results)
)
}
